...
namespace NuclearOptionMouseAim
{
    ...
    internal static class Cfg
    {
        ...
        public static ConfigEntry<bool>  RelativeTurnLead;      // v0.83: lead on the RELATIVE (nose - marker) azimuth rate, not the absolute one (A/B lever)
        ...
        public static void Bind(ConfigFile cf)
        {
            ...
            MarkerRateFeedForward = cf.Bind("Control", "MarkerRateFeedForward", true,
                "Marker-rate feed-forward (v0.78). While the marker SWEEPS at a steady rate the turn loop is pure proportional — the only integral term is gated to the fine cone and is flat zero out at a 9-degree error — so the loop can only produce a turn rate by HOLDING an error to generate it. Measured: the aim marker sweeping at 12.07 deg/s, the aircraft matching it exactly, and a rock-steady 9.54 deg of azimuth lag behind it, with nothing saturated (5.2 g of a 9 g limit, 7.7 deg of a 27 deg AoA ceiling). ON adds the marker's own measured azimuth rate straight into the commanded turn rate, so that rate no longer has to be bought with standing error — gain is exactly 1.0 (matching the marker is kinematics, not a tuning constant) and it is bounded by the same probed per-airframe achievable-rate cap as everything else, so it can never ask for a turn the aircraft cannot fly. Contributes EXACTLY zero whenever the marker is stationary, so pointing, capturing and holding are untouched; only sustained tracking of a moving marker changes. This is a checkbox rather than a rebuild so the change can be A/B'd inside one session with no restart and no DLL swap: fly a set of runs ON, toggle here, fly the same runs OFF, everything else bit-identical. The aimRate CSV column is recorded on BOTH sides.");
            RelativeTurnLead    = cf.Bind("Control", "RelativeTurnLead", true,
                "Relative-rate turn lead (v0.83). TurnLeadTime subtracts noseHeadingRate*lead from the aim error before it becomes a bank target — the v0.51 death-wobble fix. But the error's own rate of change is (markerRate - noseRate), so subtracting the ABSOLUTE nose rate is only the true derivative when the marker is STANDING STILL. While tracking a marker that sweeps at a steady rate the nose is deliberately rotating AT the marker, and the lead was subtracting that tracking rotation as if it were overshoot: measured, it removed 84% of a real 9.3 degree error, and it cancelled 60% of the v0.78 marker-rate feed-forward (the lead's gain against the error is TurnLeadTime*AssistTurnRateGain = 0.6, working directly against a feed-forward whose gain is 1.0). ON leads on the RELATIVE rate (nose rate minus marker rate) instead, which IS the error's derivative — so the term becomes true PD damping: exactly the same behaviour as before against a stationary marker (marker rate is then zero), and no braking at all in a matched sustained turn, where the standing error finally sees the full configured gain. Bounded on both sides by the existing brake/floor clamp, so it can never command more bank than the raw error justifies. The leadDeg CSV column records the lead ACTUALLY applied on both sides of this toggle.");
