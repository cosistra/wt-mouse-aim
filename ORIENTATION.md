...
| 3 | **`debugtests/SESSION-2026-08-02.md`** | The newest complete state-of-the-world: what became known, what shipped, **what data died**. | **Supersedes every older findings doc on any point they disagree.** Read §3 ("what data died") before quoting any pre-R40 number. |
...
| 6 | **`LAW-LEDGER.md`** | ESTABLISHED / PLAUSIBLE / REFUTED / OPEN, with a citation per line. | The arbiter of **what we are entitled to believe**. If a claim is not in ESTABLISHED, do not build on it. |
| 7 | **`GENERALITY-REVIEW.md`** | The standing ONE-LAW audit — every constant that should be a probe. Findings 1–18. | For ONE-LAW compliance. Its per-finding verdicts win over its (stale, v0.65-era) summary blocks. |
| 8 | **`AIRFRAMES.md`** | The 14 real jsonKeys + Vstall/Vmax/corner/gLimit/mass, and six traps in the underlying fields. | **Before writing any card `airframe` list or `startSpeed`.** Nothing else in the repo records this data. |
| 9 | **`cards/README.md`** → then `python debugtests/check-card.py cards/*.json` | The card grid, the field table, and the `sel[0]` rule. | Before adding or flying a card. Run the checker — it is the cheapest check in the repo. |
| 10 | **`debugtests/CAPTURES-DB.md`** | Column-by-column reference for `captures.db`, the metric × segment-type matrix, and the traps. | **Before writing any SQL.** Every trap in that schema returns a plausible number instead of an error. |
| 11 | `debugtests/R39-*.md`, `debugtests/R40-*.md` | The newest per-batch evidence. | As primary evidence for their own batch only. |

Older `R21`–`R37-FINDINGS.md` are historical: read them for mechanism, never for a current number.
`FLIGHT-PROTOCOL.md` is the Gate A–D instrument-validation record and nothing else — do not fly from it.
...
- **The four standing test cases.** Check every law change against all four: a **light jet at high q**;
  ...
  **hovering helo**. Two are currently unmeasurable — the rotorcraft branch never executed, and the STOL
  card flew at 2.1–2.4× corner. Do not claim coverage you do not have.
...
at **#46**; the working task list reached **#80**; `CHANGELOG.md` and `SESSION-2026-08-02.md` cite
...
`DOC-AUDIT.md` records the 2026-08-02 staleness audit of every markdown file: verdict, reason and
evidence per file, plus what was renamed and why.
