...
flowchart TB
    ...
    subgraph OUT["🟩 ARTIFACTS & OFFLINE TOOLS"]
        ...
        pytool["debugtests/analyze-wobble.py<br/>--digest · scoring · --selftest<br/>debugtests/scorecard.py<br/>per-segment metrics → score.json (v0.69) · --selftest<br/>v0.97.1: fixedWindowOffDeg · settleTime95 · offFloorPct<br/>replace terminalOffDeg for ranking — it is end-anchored<br/>(nothing settles under 9 s) AND floored (off quantum 0.0198°)<br/>debugtests/compare-runs.py<br/>cross-run spread, grouped by airframe+card+arm (v0.90)<br/>--summary = one line per card+segment · --selftest<br/>debugtests/gatechatter.py<br/>gate rail dwell/crossings + REGRESSING coincidence<br/>(MH-stratified, sham-gate control) · --selftest<br/>debugtests/loopaudit.py<br/>self-referential loops: can the command a term<br/>gates move that term? (v0.85) · --selftest<br/>debugtests/test-board-math.py<br/>extracts the BOARD-MATH region from<br/>ScenarioPlayer.cs VERBATIM, compiles it with the<br/>.NET SDK and runs 23 cases (v0.90) — it checks the<br/>shipped code, not a Python copy that would drift<br/>debugtests/test-card-model.py<br/>same trick on the CARD-MODEL region: compiles it<br/>against the game's Newtonsoft and round-trips every<br/>cards/*.json (v0.90.1). Exists because JsonUtility<br/>silently dropped Seg[] segments for nine versions<br/>+ one SYNTHETIC card for the fields no shipped card<br/>uses: config, and v0.91's comma-list airframe/count<br/>debugtests/test-arm-schedule.py<br/>same trick on TWO regions (v0.94): ARM-SCHEDULE in<br/>ScenarioPlayer.cs + ARM-SEAM in ChaseController.cs.<br/>ABBA sequence · equal MEAN POSITION at every<br/>multiple of 4 · the arm surviving a rebuilt<br/>controller · two aircraft on opposite arms at once.<br/>+ 5 source asserts (v0.96): Forget must NOT clear<br/>the arm, For must seed it, ApplyArm writes no<br/>global, all 6 lever sites read through Arm(),<br/>and LEVERS == Cfg.cs's '(A/B lever)' set<br/>debugtests/test-spec-grammar.py<br/>same trick on SPEC-GRAMMAR (SplitSpec) in<br/>ScenarioPlayer.cs, 16 cases run against BOTH it<br/>and scorecard.py's split_spec copy (v0.96)<br/>debugtests/test-fleet-resolve.py<br/>same trick on FLEET-RESOLVE (ResolveCount+CountKeys<br/>in ScenarioPlayer.cs; AirframeList+AirframeForLane<br/>in TestDrone.cs) + ENTRY-MARGINS (Stall/VMaxMargin).<br/>CountKeys(s) == len(AirframeList(s)) · lane wrap ·<br/>the 1..16 clamps · StallMargin stays BELOW 1.20 (v0.96)<br/>debugtests/test-lane-frame.py<br/>the lane base is a GlobalPosition and is converted<br/>at the LAUNCH instant, not cached at the keypress —<br/>plus the frame arithmetic: broken reproduces R35's<br/>origDist medians (against a FROZEN 8 km abeam, the<br/>constant those batches flew), fixed gives a uniform<br/>6 km ladder (v0.97.1)<br/>v0.99 also models the RING: radius at 1/2/3/8/16<br/>lanes per ring, in-deck chord and 3-D pair<br/>separation >= LaneM over N=2..16 x 6 deck spreads,<br/>|pos-base| equal across lanes at t=0 AND after<br/>31.5 km (with the shared-heading counterfactual<br/>asserted to smear, so it is not vacuous), the<br/>_slot-overflow guard, and the deck x airframe<br/>PROPERTY (every airframe on both decks, k%2 and<br/>(k/A)&amp;1 asserted to fail, deck x arm 2x2 balanced)<br/>debugtests/flightscore.py<br/>physics-normalized flight quality — every<br/>normalizer from the sidecar probe + live state,<br/>never a constant. Owns opposed(), the ONE<br/>roll/yaw cross-fight predicate · --selftest<br/>debugtests/index-captures.py<br/>SQLite cross-batch index: one row per capture, one<br/>per (capture, segment). EVERY metric comes from<br/>scorecard.score_run — one tag→metric table.<br/>--stats/--check/--diff/--cards/--archive/--query<br/>(read-only by default) · --selftest<br/>reference: debugtests/CAPTURES-DB.md<br/>debugtests/index-decompiled.py<br/>type/member index over the 0.34.1 decompile —<br/>--at NNNNN reverses a :NNNNN citation (v0.97.1).<br/>1655 types · cache lives BESIDE the decompile,<br/>never in the repo · --selftest re-derives 16 citations<br/><b>debugtests/check-card.py (v1.0.0)</b><br/>CARD PREFLIGHT — refuses a card that cannot fly its<br/>own experiment, BEFORE the game launches. Segment<br/>tags vs scorecard's table · envelope + DENSITY-<br/>corrected stall at the card's own altitude · the FBW<br/>authority knee · throttle floor · alpha reachability<br/>· wall clock. Three cards in two days failed on<br/>arithmetic it now catches. Every constant is PARSED<br/>from Cfg.cs / ScenarioPlayer.cs / TestDrone.cs /<br/>AIRFRAMES.md and it hard-errors if a regex stops<br/>matching — a hardcoded copy would be the drift it<br/>exists to catch · --selftest<br/><b>debugtests/test-helo-probe-order.py (v1.0.0)</b><br/>the v0.58 helo probe fires in the DRONE call order.<br/>(1) source: the retry structure + the three liveness<br/>columns wired end to end. (2) model: the trigger in<br/>Python over all three real call orders, with the<br/>PRE-FIX trigger as a counterfactual that MUST fail<br/>the drone case — without it the test would pass<br/>against the bug. No compiler sees a call order<br/><b>debugtests/test-card-declared.py (v1.0.0)</b><br/>the declared-zero rule: Card.Unset / Card.Declared.<br/>A SCAN, not a case table — fails on ANY comparison<br/>against zero of the four names in the two files that<br/>own them, because one more 'p.StartSpeed &gt; 0f'<br/>compiles, flies, scores, and restores the whole<br/>defect for that path. Stdlib, no SDK, no game<br/><b>debugtests/test-collective-hold.py</b><br/>same extract-and-compile trick on the<br/>COLLECTIVE-HOLD region of TestDrone.cs: the PI<br/>altitude hold that gives a rotorcraft hover card<br/>its collective. 11 one-step cases pin the SIGN and<br/>the shipped gains (an inverted collective term does<br/>not wobble, it flies the aircraft into the ground);<br/>6 closed-loop cases fly a first-order rotorcraft for<br/>120 s whose hover collective the loop is NEVER TOLD,<br/>over 0.35..0.90 — the ONE-LAW property, asserted<br/>rather than argued. Deleting the integrator drops<br/>them 89..282 m off target, which is the measured<br/>reason the loop is a PI and not a P<br/><b>debugtests/test-lane-respawn.py (v1.0.2)</b><br/>a lane that DIES and respawns recovers its<br/>replicates without a new confound. Compiles<br/>LANE-CONTINUITY + ARM-SCHEDULE together, because<br/>the property is compositional: for every death<br/>point it must fly every queue index an undamaged<br/>lane would, and NO SCORED row may be the first<br/>placement of its suite (anchor-capturing, ledger<br/>X27) — with the cost pinned at exactly one<br/>replicate per respawn, so unscoring the whole<br/>resumed tail would fail too. Plus the<br/>HARD CAP (a lane dying every replicate relaunches<br/>exactly MaxLaneRespawns times, not 16) and 5 source<br/>asserts: LaneLost asks the card player, BOTH removal<br/>paths ask before ForgetState, the respawn reuses<br/>LaunchLane, the panic key does not respawn, and the<br/>resume reaches StartSuite"]
...
`thr` (v0.77) already records the commanded throttle.
