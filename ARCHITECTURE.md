...
flowchart TB
    ...
    subgraph MOD["🟦 MOD — NuclearOptionMouseAim"]
        ...
        scenario["<b>scenario</b><br/>ScenarioPlayer<br/>test cards: play · record · select<br/>re-establishes the entry condition per replicate<br/>(anchor: pos + heading + speed + alt + fuel,<br/>demand written, controller dropped)<br/>A/B arms interleaved ABBA<br/>(off unless a card is running)"]
    ...
    scenario -->|"rigid move of ALL part rbs to the<br/>ANCHOR (pos + heading) · fuel<br/>+ zero Pilot.velocityPrev<br/>(entry force only)"| aircraft
    seam --> chase
...
destroys them again. **Phase 1 is the harness only** — the built-in level-hold still flies them and
exists solely to prove the inputs land. Both structural blockers to phase 2 are now gone: **v0.82**
made `chase` one instance per aircraft, and **v0.86** did the same for `scenario` and `telem`, so N
drones can each fly their own card and write their own CSV. What is left for phase 2 is the wiring
itself — point `Drone.Fly` at `ChaseController.For(ac).Apply(ac)`, and give `Apply` the drone's
`ScenarioPlayer.For(ac).AimDemand` in place of `AimRig.AimForward` (that marker is the *player's*, one
per process). Until then a drone's card would tick and tag rows with nothing chasing the demand.
...
flowchart TB
    ...
    look --> card["<b>ScenarioPlayer.For(ac).Tick(ac)</b><br/>THIS drone's card, written HERE so it gets the<br/>same zero-tick property the player's card gets<br/>from the seam prefix: the demand for this step<br/>lands immediately before Fly reads it"]
    card --> fly["<b>Drone.Fly(d)</b> — the PHASE-2 HOOK.<br/>Per DRONE, not one static: N drones need N<br/>independent controllers. Default = LevelHold.<br/>v0.82 cleared the blocker on the other side —<br/>ChaseController.For(ac) now returns N of those"]
    ...
    classDef mod fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
    classDef plat fill:#78350f,stroke:#fbbf24,color:#fffbeb
    class key,cap,pend,ft,due,sp,prune,gate,g1,g2,g3,call,assert,reg,post,idle,out1,look,card,fly,filt mod
    class tick plat
```
