...
flowchart TB
    ...
    scenario -->|"rigid move of ALL part rbs to the<br/>ANCHOR (pos + heading) · fuel · velocity<br/>+ zero Pilot.velocityPrev<br/>(entry force only)<br/>v0.96.1: the move AUDITS ITSELF — any part that<br/>did not follow is snapped back, because a part<br/>left behind is a DETACHED part one step later"| aircraft
    seam --> chase
...
| `Aircraft.partLookup[].rb` | **written** (public) — every part rigidbody gets the SAME rigid transform as the root, so no joint sees a relative change. **v0.96.1: and the move then re-reads `partLookup[].xform` to check that every part actually followed**, snapping back anything past `PartSnapTol` (0.10 m). A part left behind is not a cosmetic miss: `AeroPart.CheckAttachment` (`:74158`) detaches on 0.5 m of *geometry* alone, and it is the ONLY detach route an aircraft's parts have (`UnitPart.TakeDamage`'s clause excludes AeroParts, `:84095`) | `scenario` entry force, `sandbox` placement (the same `MoveAssembly`) |
| `Aircraft.partLookup[].xform` | **read**, and **written** only as the heal above, for a part that shares the root body and therefore has no rigidbody of its own to move | `scenario`/`sandbox` placement (`MoveAssembly`) |
